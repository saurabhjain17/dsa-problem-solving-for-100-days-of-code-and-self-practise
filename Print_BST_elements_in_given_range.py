...
def inorder(root,low,high,m):
    if root==None :
        return m
    elif root:
        inorder(root.left,low,high,m)
        if root.data>=low and root.data<=high:
            m.append(root.data)
        inorder(root.right,low,high,m)
    return m    
        
def printNearNodes(root, low, high):
    m=[]
    m=inorder(root,low,high,m)
    return m
